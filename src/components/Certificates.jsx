...
const Certificates = () => {
  return (
    <section id="certificates" className="py-24 relative z-10 bg-slate-900/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {certificates.map((cert, index) => (
            <motion.div
              ...
              whileHover={{ y: -10 }}
              className="glass rounded-xl overflow-hidden group border border-white/5 hover:border-[#38bdf8]/40 transition-all duration-300"
            >
              <div className="relative overflow-hidden h-48">
                ...
                <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm">
                  <a 
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-6 py-2 bg-gradient-to-r from-[#0ea5e9] to-[#38bdf8] text-white rounded-full font-medium flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 shadow-[0_0_15px_rgba(56,189,248,0.4)]"
                  >
                    View <FiExternalLink />
                  </a>
                </div>
              ...
              <div className="p-6">
                <h3 className="text-xl font-bold mb-2 text-slate-200">{cert.title}</h3>
                <p className="text-[#38bdf8] text-sm font-medium">{cert.issuer}</p>
              </div>
            </motion.div>
          ))}
        </div>
